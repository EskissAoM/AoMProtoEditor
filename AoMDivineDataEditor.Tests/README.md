# AoM Divine Data Editor regression tests

This test project intentionally lives inside the main `AoMDivineDataEditor` folder.
The desktop project excludes `AoMDivineDataEditor.Tests/**/*.cs` from its own compile items.

Run the full regression suite from the `AoMDivineDataEditor` folder:

```powershell
dotnet test .\AoMDivineDataEditor.Tests\AoMDivineDataEditor.Tests.csproj
```
